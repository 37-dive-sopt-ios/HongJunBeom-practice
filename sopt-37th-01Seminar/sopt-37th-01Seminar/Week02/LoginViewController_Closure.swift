...
class LoginViewController_Closure: UIViewController {
    ...
    private func presentToWelcomeVC() {
        ...
        vc.name = emailTextField.text //여기서 text를 텍스트로 정의해준거처럼
        vc.completionHandler = { [weak self] message in
//            guard let self else { return } 함수로 묶여서 앞에 self 하나만 self는 weak self라 있을수도있고 없을수도있어서 self를 씀
            self?.handleCompletion(message: message) //뺐을 때는 self? 이렇게 한번에 되는건가?
        } // 클로저는 클로저를 정의해줌 뭘할지
